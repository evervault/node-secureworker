...
// Input is an arraybuffer containing the quote, Subscription Key and a callback function structured at function(result,err)
...
SecureWorker.getRemoteAttestation = function getRemoteAttestation(quote, subscription_key, callbackFn) {
  var self=this;
  
  var quote_data = self.getQuoteData(quote);
  var quote_str = String.fromCharCode.apply(null,new UInt16Array(quote_data));

  var ra_url = "https://api.trustedservices.intel.com/sgx/attestation/v3/report";
  const payload = {"isvEnclaveQuote":quote_str};
  ...
  const handleResponse = function(res) {
    if(res) {
      ...
      case 200:
        ...
        if(ra_sig && ra_sig.length > 0){
          
          //convert ra_sig to an arraybuffer
          // 2 bytes for each char
          var sig_buffer = new ArrayBuffer(ra_sig.length*2);
