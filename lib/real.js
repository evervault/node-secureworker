...
// Input is an arraybuffer containing the quote, and a callback function structured at function(result,err)
...
SecureWorker.getRemoteAttestation = function getRemoteAttestation(quote,callbackFn) {
  var self=this;
  var quote_data = self.getQuoteData(quote);

  var quote_str = String.fromCharCode.appply(null,new UInt16Array(quote_data));

  const ra_url = "https://api.trustedservices.intel.com/sgx/attestation/v3/report";
  var payload = {"isvEnclaveQuote":quote_str};

  //populate Ocp-Apim-Subscription-Key with the subscription key from the API portal
  var options = {
    method: "POST",
    headers: {
  ...
  var handleResponse = function(res){
    if(res){
      ...
      case 200:
        var ra_sig = res.headers["X-IASReport-Signature"];
        if(ra_sig && ra_sig !== ""){
          //convert ra_sig to an arraybuffer

          var sig_buffer = new ArrayBuffer(ra_sig.length*2); // 2 bytes for each char
        var bufView = new Uint16Array(sig_buffer);
        for (var i=0; i < ra_sig.length; i++) {
            bufView[i] = ra_sig.charCodeAt(i);
        }
        callbackFn(sig_buffer);
        }
        break;
      case 400:
  ...
  var req = http.request(ra_url, options, handleResponse);
  req.write(payload);
  req.end();
  return;
};
